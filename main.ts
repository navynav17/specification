  ...
  try {
    ...
    const result = await page.evaluate(`(() => {
      ...
      for (const root of roots) {
        ...
        // Daraz's actual specification rows:
        // ul.specification-keys > li.key-li > span.key-title + div.key-value
...
async function main() {
  ...
  console.log(`SPEC_ROOT | found=${extracted.rootFound} | count=${extracted.rootCount}`);
  ...
  if (!Object.keys(extracted.specs).length) {
    await Actor.pushData({
      ...
      diagnostic: { rootFound: extracted.rootFound, rootCount: extracted.rootCount, specFound: extracted.specFound, sectionText: extracted.sectionText }
    });
